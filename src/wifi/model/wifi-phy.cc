...
WifiPhy::GetPlcpHeaderMode (WifiTxVector txVector)
{
  switch (txVector.GetMode ().GetModulationClass ())
    ...
    case WIFI_MOD_CLASS_IR:
      return WifiPhy::GetOwcRate10Gbps();
    default:
...
WifiPhy::GetPayloadDuration (uint32_t size, WifiTxVector txVector, uint16_t frequency, MpduType mpdutype, uint8_t incFlag)
{
  ...
  switch (payloadMode.GetModulationClass ())
    ...
    case WIFI_MOD_CLASS_VHT:
      {
        if (payloadMode.GetModulationClass () == WIFI_MOD_CLASS_HT && Is2_4Ghz (frequency)
          ...
          {
            //std::cout << "femto: " << FemtoSeconds (numSymbols * symbolDuration.GetFemtoSeconds ()) << std::endl;
            return FemtoSeconds (numSymbols * symbolDuration.GetFemtoSeconds ());
          }
      }
    case WIFI_MOD_CLASS_HE:
    ...
    case WIFI_MOD_CLASS_IR:
      //std::cout << "calc: " << (size * 8.0) / (payloadMode.GetDataRate (22) / 1.0e6) << std::endl;
      //std::cout << "calc2: " << MicroSeconds (lrint (ceil ((size * 8.0) / (payloadMode.GetDataRate (22) / 1.0e6)))) << std::endl;
      //return FemtoSeconds (numSymbols * symbolDuration.GetFemtoSeconds ());
...
WifiMode
WifiPhy::GetOwcRate10Gbps ()
{
  static WifiMode mode =
    WifiModeFactory::CreateWifiMode ("OwcRate10Gbps",
                                     WIFI_MOD_CLASS_IR,
                                     true,
                                     WIFI_CODE_RATE_UNDEFINED,
                                     2);
...
WifiMode
WifiPhy::GetOwcRate1Gbps ()
{
  static WifiMode mode =
    WifiModeFactory::CreateWifiMode ("OwcRate1Gbps",
                                     WIFI_MOD_CLASS_IR,
                                     false,
                                     WIFI_CODE_RATE_UNDEFINED,
                                     2);
...
public:
  Constructor ()
  {
    ...
    ns3::WifiPhy::GetOwcRate10Gbps ();
    ns3::WifiPhy::GetOwcRate1Gbps ();
  }
