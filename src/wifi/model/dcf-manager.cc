...
DcfManager::SetSlot (Time slotTime)
{
  ...
  m_slotTimeUs = slotTime.GetNanoSeconds ();//sampath MicroSeconds
}
...
DcfManager::IsWithinAifs (DcfState *state) const
{
  ...
  Time ifsEnd = GetAccessGrantStart () + NanoSeconds (state->GetAifsn () * m_slotTimeUs);//sampath
...
DcfManager::GetBackoffStartFor (DcfState *state)
{
  ...
  Time mostRecentEvent = MostRecent (state->GetBackoffStart (),
                                     GetAccessGrantStart () + NanoSeconds (state->GetAifsn () * m_slotTimeUs));//sampath
...
DcfManager::GetBackoffEndFor (DcfState *state)
{
  ...
  NS_LOG_DEBUG ("Backoff start: " << GetBackoffStartFor (state).As (Time::US) <<
                " end: " << (GetBackoffStartFor (state) +
                             NanoSeconds (state->GetBackoffSlots () * m_slotTimeUs)).As (Time::US));//sampath
  return GetBackoffStartFor (state) + NanoSeconds (state->GetBackoffSlots () * m_slotTimeUs);//sampath
}
  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); i++, k++)
    {
      ...
      if (backoffStart <= Simulator::Now ())
        {
          uint32_t nus = (Simulator::Now () - backoffStart).GetNanoSeconds ();//sampath
          ...
          Time backoffUpdateBound = backoffStart + NanoSeconds (n * m_slotTimeUs);//sampath
