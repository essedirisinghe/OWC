...
double
NistErrorRateModel::GetFecOokBer (double snr, uint32_t nbits) const
{
  NS_LOG_FUNCTION (this << snr << nbits);
  double z = std::sqrt (snr);
  double ber = 0.5 * erfc (0.5*z);
  
  NS_LOG_INFO ("ook snr=" << snr << " ber=" << ber);
