...
YansWifiPhy::StartTx (Ptr<Packet> packet, WifiTxVector txVector, Time txDuration)
{
  ...
  ///std::cout << "packet info: " << hdr.GetTypeString() <<std::endl;
  
  NS_LOG_DEBUG ("Start transmission: signal power before antenna gain=" << GetPowerDbm (txVector.GetTxPowerLevel ()) << "dBm" << " duration"<<txDuration);
